...
class test_user(TestCase):
    ...
    def test_usuario_eliminar(self):
        ...
        """ Asignamos a la variable user el usuario sicp, osea el administrador.
        """
    ...
    def testUsuarioNuevo(self): # Create an instance of a GET request. 
        self.user = User.objects.get(pk=1) 
        request = self.factory.post('/adm_usuarios/nuevo/', {'Nombre_de_Usuario': 'usuario2', 'Contrasenha': 'ysicpy', 'Confirmar_contrasenha': 'ysicpy', 'Email': 'ysicpy', 'Nombre': 'ysa', 'Apellido':'ortiz', 'Telefono': '021', 'Direccion': 'lambare'})
        request.user = self.user 
    ...
    def testUsuarioBuscar(self):  
        self.user = User.objects.get(pk=1) 
        request = self.factory.get('/adm_usuarios/', {'busqueda': 'ysicpy'})
        request.user = self.user
    ...
    def testModificarUsuario(self):
        ...
        usuario = User.objects.get(username='sicp')
        request = self.factory.post('/adm_roles/modificar/1/', {'Nombre_de_Usuario': 'sicpMod', 'Contrasenha': '', 'Nueva_contrasenha': '', 'Email': usuario.email, 'Nombre': usuario.first_name,'Apellido': usuario.last_name, 'Telefono': '2333443545', 'Direccion' : 'xxxxxxxxxxx', 'Especialidad' : '' , 'Observaciones' : ''})
        request.user = self.user
